    ...
    if (peliculas.length >= 1) {
        $.each(peliculas, function (index, lavadora) {
            ...
            contenido += '<tr><th scope="row">' + lavadora.id + '</th>' +
                    ...
                    '<td><input type="checkbox" name="novedad" id="novedad' + lavadora.id
                    + '" disabled ';
            if (lavadora.novedad) {
                contenido += 'checked'
            }
            contenido += '></td><td>' + lavadora.fechaAlquiler + '</td>' +
                    '<td><button id="devolver-btn" onclick= "devolverpelicula(' + lavadora.id
